...
import (
	...
	"pikabu-signaling/signaling/pkg/service"
	"runtime"
	"strings"
	"time"
)

var fs = flag.NewFlagSet("pikabu-signaling", flag.ExitOnError)
...
func Register() {
	...
	myIp, _ := service.ExternalIP()
	message := pb.RegisterRequest{
		Account: tokens[1],
		Type: randomdata.Adjective(),
		Ip: myIp,
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "pb.RegisterRequest error: %v", err)
		return
	} else {
